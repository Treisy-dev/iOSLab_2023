...
class ProfileDataManager: NSObject, DataManagerProtocol, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        syncGetAllPosts(forUser: LogInDataManager.authUser).count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let posts = syncGetAllPosts(forUser: LogInDataManager.authUser)
    ...
    func asyncDelete(_ model: Post, completion: @escaping (Result<Void, Error>) -> Void) {
        let operation = BlockOperation {
            ...
            if let index = self.postsDictionary[LogInDataManager.authUser]?.firstIndex(where: { $0.id == model.id }) {
                self.postsDictionary[LogInDataManager.authUser]?.remove(at: index)
            }
        }
